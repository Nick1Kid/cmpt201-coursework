...
void groupByKey(IntermediateInput *input, Output *output, int *result_count) {
  ...
  if (!found) {
    int new_index = group;
    output[new_index].doubled_value = input->doubled_value;
    output[new_index].line_numbers[0] = input->line_number;
    output[new_index].count = 1;
    *result_count = group + 1;
  }
}
